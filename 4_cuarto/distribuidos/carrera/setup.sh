...
usage() {
    cat <<EOF
Uso: $(basename "${BASH_SOURCE[0]}") [-h] [-s serverIP] host1IP [host2IP...]

Script para desplegar el entorno en varios hosts.
...
parse_params() {
    ...
    [[ -z "${server-}" ]] && [[ ${#args[@]} -eq 0 ]] && echo "Hay que especificar al menos un parametro" && exit 1
...
if [ -n "${server-}" ]; then
    if [ "${server}" = "127.0.0.1" ] || [ "${server}" = "localhost" ] ||
        ...
        if ! nc -z "${server}" 22 2>/dev/null; then
            ...
            ssh "${server}" "cd ${remote_script_dir} && ./$(basename "$0") -s localhost"
        fi
    fi
fi
...
for host in "${args[@]}"; do
    if [ "${host}" = "127.0.0.1" ] || [ "${host}" = "localhost" ] ||
        ...
        if ! nc -z "${host}" 22 2>/dev/null; then
            ...
            ssh "${host}" "cd ${remote_script_dir} && ./$(basename "$0") localhost"
        fi
    fi
done
