...
TOMCAT_VER=$(curl -k --silent https://dlcdn.apache.org/tomcat/tomcat-9/ | grep v9 | awk '{split($5,c,">v") ; split(c[2],d,"/") ; print d[1]}' | tail -n 1)
...
usage() {
    cat <<EOF
Uso: $(basename "${BASH_SOURCE[0]}") [-h] [-g gestor] server1 [server2...]
...
setup_jersey() {
    wget --no-check-certificate "${JERSEY_URL}" -O "${TMP}/jaxrs-ri.tar.gz"
...
setup_server() {
    ...
    if ! [ -d "${CATALINA_HOME}" ]; then
        wget --no-check-certificate "${TOMCAT_URL}" -O "${TMP}/apache-tomcat.tar.gz"
    ...
    if [ -n "${server-}" ]; then
        if [ "${server}" = "127.0.0.1" ] || [ "${server}" = "localhost" ] ||
            ...
            if [ "${clean}" = true ] || [ "${fullclean}" = true ]; then
                ...
                setup_makefile
                setup_server
                echo "Servidor iniciado en: $(hostname -I | awk '{print $1;}')"
            fi
        else
...
if [ -n "${host}" ]; then
    if [ "${host}" = "127.0.0.1" ] || [ "${host}" = "localhost" ] ||
        ...
        if [ "${clean}" = true ] || [ "${fullclean}" = true ]; then
            ...
            setup_makefile
            setup_client
            echo "Gestor listo para ser lanzado en: $(hostname -I | awk '{print $1;}')"
        fi
    else
        if ! nc -z "${host}" 22 2>/dev/null; then
            ...
            ssh "${host}" "cd ${remote_script_dir} && ./$(basename "$0") -g localhost"
        fi
    fi
fi
