...
public class Utils {
  ...
  protected static DFAgentDescription[] buscarAgentes(Agent agent, String tipo) {
    ...
    // como define el tipo el agente coordinador tambien podriamos buscar por nombre
    ...
    try {
      ...
      // JOptionPane.showMessageDialog(null, "Agente "+getLocalName()+": "+e.getMessage(), "Error",
  ...
  /**
   * Permite buscar los agents que dan un servicio de un determinado tipo. Devuelve el primero de
   * ellos.
   * 
  ...
  protected static DFAgentDescription buscarAgente(Agent agent, String tipo) {
    ...
    DFAgentDescription template = new DFAgentDescription();
    ServiceDescription templateSd = new ServiceDescription();
    // como define el tipo el agente coordinador tamiben podriamos buscar por nombre
    templateSd.setType(tipo);
    template.addServices(templateSd);
    ...
    try {
      ...
      if (results.length > 0) {
        ...
        for (int i = 0; i < results.length; ++i) {
          ...
          while (it.hasNext()) {
            ...
            if (sd.getType().equals(tipo)) {
              System.out.println("- Servicio \"" + sd.getName() + "\" proporcionado por el agente "
                  + provider.getName());
        ...
        // JOptionPane.showMessageDialog(null, "Agente "+getLocalName()+" no encontro ningun
        // servicio buscador", "Error", JOptionPane.INFORMATION_MESSAGE);
      }
    } catch (FIPAException e) {
      // JOptionPane.showMessageDialog(null, "Agente "+getLocalName()+": "+e.getMessage(), "Error",
      // JOptionPane.ERROR_MESSAGE);
